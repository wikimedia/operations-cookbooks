"""
Unit tests for sre.mysql.newpool
Test using:
tox -e py311-unit -- tests/unit/sre/mysql/newpool_test.py -vv
"""
...
import cookbooks.sre.mysql.newpool
from cookbooks.sre.mysql.newpool import (
    PoolDepoolRunner,
...
def mock_durable_shell_and_sleep():
    with (
        patch("cookbooks.sre.mysql.newpool.ensure_shell_is_durable", autospec=True),
        patch("cookbooks.sre.mysql.newpool.sleep", autospec=True),
    ):
        yield
...
@fixture(autouse=True)
def m_jget():
    with patch("cookbooks.sre.mysql.newpool._jget") as m:
        yield m
...
def test_last_instance_depool() -> None:
    ...
    ac = mock.MagicMock()
    cookbooks.sre.mysql.newpool.ask_confirmation = ac
...
@patch("cookbooks.sre.mysql.newpool._jget")
def test_runner_unsupported_section(m_jget, caplog) -> None:
    ...
    with raises(RuntimeError):
        ffz = cookbooks.sre.mysql.newpool.fetch_host_instance_from_zarcillo
        ffz("db2235")
...
@patch("cookbooks.sre.mysql.newpool._check_depooling_last_instance", autospec=True)
