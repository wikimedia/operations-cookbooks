...
class TlsRunner(CookbookRunnerBase):
    ...
    def get_cert(self) -> Optional[Certificate]:
        ...
        try:
            ...
            logger.info("Can't connect to device, assuming initial bootstrap.")
            return None
    ...
    def need_initial_setup(self, cert_x509: Optional[Certificate]) -> bool:
        ...
        if cert_org_name != 'Wikimedia Foundation, Inc':
            logger.info('Certificate not generated by WMF (%s)', cert_org_name)
            return True
        if cert_name != self.device_fqdn:
            logger.info("Certificate doesn't match device FQDN (%s vs. %s)", cert_org_name, self.device_fqdn)
            return True
        return False
