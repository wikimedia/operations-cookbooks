...
def parse_db_host_fqdn(fqdn: str) -> Tuple[str, str]:
    ...
    for dc_number in hostname:
        if dc_number.isdigit():
            break

    if dc_number == "1":
        ensure(dcname == "eqiad", f"Inconsistent FQDN '{fqdn}'")
    else:
        ensure(dcname == "codfw" and dc_number == "2", f"Inconsistent FQDN '{fqdn}'")
...
class CloneMySQLRunner(CookbookRunnerBase):
    ...
    def __init__(self, args, spicerack: Spicerack):
        ...
        self.section = source_section

        ensure(self.section.startswith("es"), "Only RO es* sections are supported")
        n = int(self.section[2:])
        ensure(n >= 1 and n < 6, f"Section {self.section} not supported")
    ...
    def run(self) -> None:
        ...
        source_downtime_id = source_alerter.downtime(self.admin_reason, duration=timedelta(hours=96))

        step("stop-ferm-exp", f"Stopping nrpe2nodexp-ferm_active  on {self.target_host}")
        _run(self.target_host, "systemctl stop nrpe2nodexp-ferm_active")

        target_alerter = self.alerting_hosts(self.target_host.hosts)
        target_downtime_id = target_alerter.downtime(self.admin_reason, duration=timedelta(hours=96))
        ...
        step("restart-mysqld-exporter", f"Restart prometheus-mysqld-exporter.service on {self.source_host}")
        _run(self.source_host, "systemctl restart prometheus-mysqld-exporter.service")

        step("restart-mysqld-exporter", f"Restart prometheus-mysqld-exporter.service on {self.target_host}")
        _run(self.target_host, "systemctl restart prometheus-mysqld-exporter.service")

        step("wait_icinga_s", f"Waiting for icinga to go green for {self.source_fqdn}")
        self._source_icinga_host.wait_for_optimal()

        ask_confirmation(f"Remove downtime on {self.source_fqdn} and pool it in?")

        step("restart-mysqld-exporter", f"Restart prometheus-mysqld-exporter.service on {self.source_host}")
        _run(self.source_host, "systemctl restart prometheus-mysqld-exporter.service")

        step("stop-ferm-exp", f"Removing stale ferm file  on {self.target_host}")
        _run(self.target_host, "rm /var/lib/prometheus/node.d/check_ferm_active.prom -f")

        step("stop-ferm-exp", f"Restarting nrpe2nodexp-ferm_active  on {self.target_host}")
        _run(self.target_host, "systemctl restart nrpe2nodexp-ferm_active")

        step("restart-mysqld-exporter", f"Restart prometheus-mysqld-exporter.service on {self.target_host}")
        _run(self.target_host, "sudo systemctl restart prometheus-mysqld-exporter.service")

        step("wait_icinga_s", f"Waiting for icinga to go green for {self.source_fqdn} again")
        self._source_icinga_host.wait_for_optimal()
        ...
        if self.pool_in_target:
            ...
            t0 = time.time()
            _wait_until_target_dbctl_conf_is_good(self._dbctl, self.target_hostname, self.section)
    ...
    def _run_clone(self) -> None:
        ...
        self.logger.info("Extending logical volume /dev/tank/data on %s", self.target_hostname)
        _run(self.target_host, "lvextend -l +100%FREE /dev/tank/data")

        self.logger.info("Running xfs_growfs on %s", self.target_hostname)
        _run(self.target_host, "xfs_growfs /srv")
