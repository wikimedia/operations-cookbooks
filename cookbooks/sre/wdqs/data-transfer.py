...
def open_port(host, source, port, proto='tcp'):
    ...
    &R_SERVICE([{proto}], {port} @resolve(({source})))
    &R_SERVICE({proto}, {port} @resolve(({source}),AAAA))
    """.format(port=port, source=source, proto=proto)
    host.run_sync('echo "{}" > {}'.format(dedent(ferm_rule), ferm_path))
    host.run_sync('/usr/bin/systemctl restart ferm')
    try:
        yield
    finally:
        host.run_sync('/bin/rm {}'.format(ferm_path))
        host.run_sync('/usr/bin/systemctl restart ferm')
