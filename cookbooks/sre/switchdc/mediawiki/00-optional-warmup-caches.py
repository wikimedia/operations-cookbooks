...
from cookbooks.sre.switchdc.mediawiki import MEDIAWIKI_RO_SERVICES, MediaWikiSwitchDCBase, MediaWikiSwitchDCRunnerBase
...
class WarmupCachesRunner(MediaWikiSwitchDCRunnerBase):
    ...
    def run(self):
        ...
        if self.live_test:
            ...
            datacenter = self.dc_to

        current_ro_state = self.spicerack.discovery(*MEDIAWIKI_RO_SERVICES).active_datacenters
        ...
        # urls-cluster.txt contains requests that are useful for warming up shared resources (e.g., memcache), so it
        # only needs run against a single service. urls-server.txt, in contrast, contains requests for warming local
        # resources (e.g., APCu), and is thus run against mw-web and both API services.
        ...
        deployment_host = self.spicerack.remote().query(self.spicerack.dns().resolve_cname(DEPLOYMENT_HOST))
        # It takes multiple executions of the warmup script to fully warm up the caches. The second run is faster than
        # the first, and so on. Empirically, we consider the caches to be fully warmed up when this speedup disappears;
...
class WarmupCaches(MediaWikiSwitchDCBase):
    """Warmup MediaWiki caches."""

    runner_class = WarmupCachesRunner
