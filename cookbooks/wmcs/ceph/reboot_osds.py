...
from cookbooks.wmcs.ceph.reboot_osd import RebootOsd
...
class RebootOsdsRunner(CookbookRunnerBase):
    ...
    def run(self) -> Optional[int]:
        ...
        dologmsg(project="admin", message=f"Rebooting the nodes {osd_nodes}", task_id=self.task_id)

        controller.set_maintenance()

        reboot_osd_cookbook = RebootOsd(spicerack=self.spicerack)
        for index, osd_node in enumerate(osd_nodes):
            ...
            reboot_osd_cookbook.get_runner(args=reboot_osd_cookbook.argument_parser().parse_args(args)).run()
            LOGGER.info(
